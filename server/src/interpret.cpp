...
void assert_attribute_name (const char * attribute_name) {
	for (int j = 0; j < 16; j++) {
		...
		if ('a' <= c && c <= 'z' || c == '_') {
			...
			throw std::runtime_error("encountered char '" + std::to_string(c) + "' in attribute name??");
		}
	}
}
...
public:
	EntryDescriptor (const uint64_t * const buffer, const uint64_t length_in_words) {
		self().resize(length_in_words / 2);
		// one attribute_name is stored in 2 words, 16 bytes.
		for (uint64_t offset = 0; offset < length_in_words; offset += 2) {
	...
	EntryDescriptorMap (const uint64_t * const buffer, const size_t length_in_words) {
		...
		for (uint64_t i = 0; i < type_count; i++) {
			...
			self().emplace(entry_type, EntryDescriptor (
				current_entry_descriptor_names, entry_descriptor_length
			));
			current_entry_descriptor_names += entry_descriptor_length;
		}
	}
...
public:
	Entry (
		...
		for (size_t i = 1; i < entry_descriptor.size(); i++) {
			std::string entry_name = entry_descriptor[i];
			self()[entry_name] = entry_descriptor.read_in(i, buffer, length_in_words);
		}
	}
};
	...
	EntryArray (const RawEntryArray & raw_entry_array) {
		for (size_t i = 0; i < raw_entry_array.size(); i++) {
			...
			if (*type_ptr == BTE_INFO) {
				EntryDescriptorMap entry_descriptor_map { raw_entry_array[i], entry_length };
			}
		}
	}
};
...
int main(int argc, char * argv []) {
	...
	std::basic_ifstream<uint64_t> file { filename };

	file.read(buffer, BUFFER_CAPACITY_IN_WORDS);
	if (!file) {
		std::runtime_error("failed reading from '" + filename + "'!");
	}
