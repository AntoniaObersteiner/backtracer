...
void write_out(std::string filename, std::span<const uint64_t> data) {
...
int main(int argc, char * argv []) {
	...
	size_t header_capacity_in_words = (sizeof(compression_header_t) - 1) / sizeof(uint64_t) + 1;
	if (input_size_in_words < header_capacity_in_words) {
	...
	if (!compression_header->is_compressed) {
		...
		write_out(output_filename, decompressed);
		return 0;
	}
	...
	write_out(output_filename, std::span { decompressed });
}
